    ...
    <div className={Style.listForm}>
      <div className={Style.listForminner}>
        {togetherDTO.map((item) => {
          ...
          const searchSub = subItemDTO
            .filter((subItem) => subItem.toMainSeq === item.togetherSeq)
            .find((item2) => item2.placeSw === 0);
          const searchSub_Cus = subItemDTO
            .filter((subItem_cus) => subItem_cus.toMainSeq === item.togetherSeq)
            .find((item2) => item2.placeSw === 1);
          return (
            <div
              className={Style.together}
              key={item.togetherSeq}
              onClick={() => onTogetherView(item.togetherSeq)}
              onMouseOverCapture={() => setHover(item.togetherSeq)}
              onMouseOutCapture={() => setHover(-1)}
            >
              ...
              {searchSub !== undefined &&
                place.find(
                  (placeItem) => placeItem.placeSeq === searchSub.placeSeq
                ) && (
                  <div className={Style.togetherFoot}>
                    <div className={Style.imgDiv}>
                      <img
              ...
              <div className={Style.dateTop}>
                ...
                <div
                  className={Style.userSeq}
                  onClick={(e) => onModal(e, item.userSeq)}
                >
                  ...
                  <div className={Style.userinfo}>
                    ...
                    <p className={Style.userGender}>
                      {item.userGender === 'M' ? '남성' : '여성'}
                    </p>
                  </div>
        ...
        <div
          className={Style.loadingSection}
          style={{ display: scrollLoading ? 'block' : 'none' }}
        >
          <img src={loadingImg} />
