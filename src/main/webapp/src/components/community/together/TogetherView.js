...
const TogetherView = ({ seqAd, open, onOpen }) => {
  ...
  useEffect(() => {
    getAllSubscript().then((res) =>
      setSubscripts(res.data.filter((sub) => sub.toMainSeq == togetherSeq))
    );
    ...
    getChatSeq(togetherSeq).then((res) => setChat(res.data));
  ...
  useEffect(() => {
    getTogetherSeq(seqAd === undefined ? togetherSeq : seqAd).then((res) => {
  ...
  useEffect(() => {
    sortedSubDTO.forEach((item) => {
      if (array !== null && item.placeSw === 0) {
        const placeL = place.find(
          (p) => sortedSubDTO[array].placeSeq === p.placeSeq
        );
        if (placeL) {
      ...
      if (array !== null && item.placeSw === 1) {
        const customL = custom.find(
          (c) => sortedSubDTO[array].plCustomSeq === c.plCustomSeq
        );
        if (customL) {
  ...
  const onChange = (togetherDTO, subDTO, custom, place) => {
    window.scrollTo(0, 0);
    navigate('/community/together/write', {
      state: { togetherDTO, subDTO, custom, place },
    });
  };
    ...
    sweet
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          deleteTogether(togetherSeq).then((res) => {
            window.scrollTo(0, 0);
            sweet
              .fire({
                title: '삭제되었습니다',
    ...
    if (isConfirmed) {
      ...
      sweet
        ...
        .then(
          getAllSubscript().then((res) =>
            setSubscripts(
              res.data.filter((sub) => sub.toMainSeq == togetherSeq)
            )
          )
        );
    }
      ...
      <div className={Style.viewInner}>
        ...
        {togetherDTO.userSeq === user.userSeq && (
          <div>
            ...
            <button
              className={Style.listDelete}
              onClick={() => goDelete()}
            >
              삭 제
            </button>
            <button
              className={Style.listChange}
              onClick={() => onChange(togetherDTO, subDTO, custom, place)}
            >
              수 정
            </button>
          </div>
        ...
        {place !== null && place.length > 0 ? (
          <div className={Style.midSc}>
            <div className={Style.midScTdiv}>
              <span className={Style.midScText}>지역</span> &nbsp; {cityFind}
            </div>
            <div className={Style.midScTdiv}>
              <span className={Style.midScText}>정원</span> &nbsp;{' '}
              {togetherDTO.tnum}
            </div>
            <div className={Style.midScTdiv}>
              <span className={Style.midScText}>날짜</span> &nbsp;{' '}
              {togetherDTO.startDate}~{togetherDTO.endDate}
            </div>
          </div>
        ) : (
          custom.length > 0 && (
            <div className={Style.midSc}>
              <div className={Style.midScTdiv}>
                <span className={Style.midScText}>지역</span> &nbsp;{' '}
                {custom[0].placeName}
              </div>
              <div className={Style.midScTdiv}>
                <span className={Style.midScText}>정원</span> &nbsp;{' '}
            ...
            <div className={Style.togetherList}>
              ...
              <div className={Style.togetherSchedule}>
                {sortedSubDTO.map((item, index) => (
                  <button
                    className={`${Style.subDay} ${
                      array === index ? Style.selecDay : ''
                    }`}
                ...
                {clickAdress !== null && array !== null && (
                  <div className={Style.scheduleContext}>
                    <p>
                      {sortedSubDTO[array].startTime}시~
                      {sortedSubDTO[array].endTime}시
                    </p>
                    <br />
                    <p>{clickAdress}</p>
                    <br />
          ...
          <div className={Style.togetherChatDiv}>
            <div className={Style.togetherChatInner}>
              <div className={Style.chatProfile}>
                {userList.profileImage !== null ? (
                  <div
                    className={Style.chatProfileImg}
                ...
                <div className={Style.inin}>
                  <div className={Style.chatProfileName}>
                    {togetherDTO.userid}
                  </div>
                  <div className={Style.chatProfileInfo}>
                    {togetherDTO.userGender === 'M' ? '남자' : '여자'}
                  </div>
                </div>
                {userList.likingFood && userList.likingTrip && (
                  <div className={Style.chatText}>
                    <p>
                      <span>{userList.likingFood}</span>의 음식 취향을 가진
                    </p>
                    <p>
                      <span>{userList.likingTrip}</span>를 좋아하는 여행자예요
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className={Style.togetherChatInner2}>
              {!open &&
                user.name !== '' &&
                togetherDTO.userSeq !== user.userSeq &&
                subscripts.findIndex(
                  (subs) => subs.userSeq === user.userSeq
                ) === -1 &&
                chat &&
                !chat.entrySeq.split(',').includes(user.userSeq.toString()) &&
                chat &&
                chat.entrySeq.split(',').length + (chat.entrySeq !== '' && 1) <
                  togetherDTO.tnum && (
                  <div className={Style.chatGoDiv}>
                    <button
                      className={Style.chatGo}
                      onClick={() => subscript()}
                    >
                      신청하기
                    </button>
              ...
              {chat &&
                !chat.entrySeq.split(',').includes(user.userSeq.toString()) &&
                chat.entrySeq !== '' &&
                chat.entrySeq.split(',').length + 1 >= togetherDTO.tnum && (
                  <div className={Style.chatGoDiv}>
                    <button
                      className={Style.chatGo}
                      style={{ backgroundColor: '#6c90bc' }}
              ...
              <div className={Style.toGother}>
                <div className={Style.toGotherMaster}>
                  {togetherDTO.userProfileImage ? (
                    <div
                      className={Style.toGotherImg}
                      onClick={() => onModal(togetherDTO.userSeq)}
                    >
                      <img
                        src={userList.profileImage}
                        className={Style.userImg}
                ...
                {chat &&
                  users
                    .filter((item) =>
                      chat.entrySeq.split(',').includes(item.userSeq + '')
                    )
                    .map((en) => (
                      <div className={Style.toGotherMaster}>
                        {en.profileImage ? (
